...
class TestGaussianFitter:
    ...
    def _make_synthetic_peak(self, center=50.0, sigma=2.0, amplitude=100.0,
                              background=10.0, n_points=500, energy_range=(0, 100)):
        """Create synthetic Gaussian peak with noise."""
        energies = np.linspace(energy_range[0], energy_range[1], n_points)
        counts = amplitude * np.exp(-0.5 * ((energies - center) / sigma) ** 2) + background
        counts += np.random.RandomState(42).normal(0, np.sqrt(np.maximum(counts, 1)), n_points)
    ...
    def test_resolution_fraction(self):
        ...
        np.testing.assert_allclose(result.resolution_fraction(), expected_frac, rtol=0.05)
...
class TestPeakFitter:
    ...
    def _make_multi_peak(self):
        ...
        counts += np.random.RandomState(42).normal(0, np.sqrt(np.maximum(counts, 1)), len(energies))
        return energies, counts
