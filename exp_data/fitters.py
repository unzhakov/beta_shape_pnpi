...
class GaussianFitter:
    ...
    def _gaussian_background(self, params: np.ndarray, x: np.ndarray) -> np.ndarray:
...
class PeakFitter:
    ...
    def find_peaks(
        ...
        indices, _ = find_peaks(
            heights,
            height=min_height,
            distance=int(min_distance_keV / max(1.0, np.diff(self.energies).min())),
        )
    ...
    def fit_peaks(
        ...
        if known_energies is not None:
            ...
            peak_energies = np.array([self._nearest_channel(e) for e in known_energies])
