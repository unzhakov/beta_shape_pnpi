...
Usage examples:
    ...
    bs_pnpi --nuclide Tc99 --output spectrum.csv -vv --log-file calc.log
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    log_group.add_argument(
        ...
        type=str,
        metavar="FILE",
        help="Append log messages to the specified file.",
    )
    log_group.add_argument(
        "--dry-run",
        action="store_true",
...
def _run(args: argparse.Namespace) -> None:
    ...
    source_type: str
    if args.nuclide:
        logger.info("Source: paceENSDF nuclide=%s mode=%s", args.nuclide, args.mode)
    ...
    if args.transition_type:
        ...
        logger.info("Override transition_type=%s", args.transition_type)
    ...
    logger.info(
        "Creating BetaSpectrum for %s -> Z=%d,%d, A=%d, endpoint=%.3f MeV",
        _element_symbol(config.Z_parent),
        config.Z_parent,
        config.Z_daughter,
        config.A_number,
        config.endpoint_MeV,
...
def _print_dry_run_output(config, source_type: str) -> None:
    ...
    print(f"Nuclide:    {config.Z_parent} -> {config.Z_daughter}, A={config.A_number}")
