...
$$R(W,W_0) = 1 + \\frac{\\alpha}{2\\pi}\\left[g(W_0,W) - 3\\ln\\left(\\frac{m_p}{2W_0}\\right)\\right] + Z\\alpha^2\\sum_i \\Delta_i(W) + Z^2\\alpha^3 \\delta\_{3h} + ... \\tag{48}$$
...
$$L_s(x) = \\int_0^x \\frac{\\ln(1-t)}{t} dt = -Li_2(x) = -\\sum\_{k=1}^\\infty \\frac{x^k}{k^2} \\tag{51}$$
...
To handle the endpoint divergence, the leading logarithmic term $C(\\beta)\\ln(W_0 - W)$ is replaced with an exponentiated form:
...
**Implementation note:** The resummation is only applied when $W_0 - W < \\delta\_{\\text{cut}}$ (default $\\delta\_{\\text{cut}} = 10^{-3}$ in $m_e$ units, ≈ 0.5 keV). For $W_0 - W \\gg \\delta\_{\\text{cut}}$, the standard logarithmic form is used. This ensures resummed and standard modes are identical far from the endpoint.
...
$$\\delta_2(Z,W) = Z\\alpha^2 \\sum\_{i=1}^4 \\Delta_i(W) \\tag{55}$$
...
$$\\delta\_{3h} = Z^2\\alpha^3 \\left[a\\ln\\left(\\frac{\\Lambda}{W}\\right) + bf(W) + \\frac{4\\pi}{3}g(W) - 0.649\\ln(2W_0)\\right]$$

with $a = \\frac{\\pi}{3} - \\frac{\\sqrt{3}}{2}$, $b = \\frac{4}{3\\pi^4}(\\pi^2 - \\gamma_E) - \\frac{\\pi^2}{18}$.

### Higher Order (Eq. 68–69)

$$\\delta\_{Z^n\\alpha^{n+1}} \\approx Z^n\\alpha^{n+1} K\_{nm} \\ln^{m-n}\\left(\\frac{\\Lambda}{W}\\right), \\quad K\_{nm} \\approx 0.5 \\tag{68}$$
$$\\sum\_{n=3}^\\infty Z^n\\alpha^{n+1} = \\frac{Z^3\\alpha^4}{1-Z\\alpha} \\tag{69}$$
...
Experimental branching ratios: $BR\_{\\beta\\gamma} \\sim 3\\times 10^{-3}$ (neutron), $\\sim 2\\times 10^{-3}$ ($^{32}$P).

## Calculator Implementation

> [!tip] Code mapping
> Module: `beta_spectrum/components/radiative.py` → `RadiativeCorrection(W0, Z, A, use_endpoint_resummation, delta_cut)` class
>
...
> - Soft-photon resummation near endpoint via Sirlin (1987) prescription (only when $W_0 - W < \\delta\_{\\text{cut}}$)
...
# Light nucleus (tritium-like) — resummation has negligible effect at low W0
...
rad = RadiativeCorrection(
    ...
    use_endpoint_resummation=True,
    delta_cut=1e-3  # only apply resummation within 1 keV of endpoint
)

# Standard (non-resummed) mode for comparison
rad_standard = RadiativeCorrection(
    W0=5.0,
    Z=92,
    use_endpoint_resummation=False
)

values = rad(W_grid)
```

> [!note] Resummation scope
> The soft-photon resummation (Eq. 53) is only applied when $W_0 - W < \\delta\_{\\text{cut}}$ (default $10^{-3}$ in $m_e$ units ≈ 0.5 keV). Far from the endpoint, both resummed and standard modes produce identical results. This matches the Sirlin (1987, 2011) prescription where resummation replaces only the leading logarithmic divergence near $W \\to W_0$.
...
| $Z^2\\alpha^3 \\delta\_{3h}$ | < 0.01%      | Only for heavy nuclei            |
